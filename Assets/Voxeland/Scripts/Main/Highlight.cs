...
namespace Voxeland {
	...
	public class Highlight : MonoBehaviour 
	{
		...
		{ 
			...
			//placing transform
			filter.transform.localPosition = new Vector3(face.x+0.5f+chunk.offsetX, face.y+0.5f, face.z+0.5f+chunk.offsetZ);// + chunk.land.transform.position;

			//creating mesh
			Vector3 addVector= new Vector3(-face.x-0.5f, -face.y-0.5f, -face.z-0.5f);
			for (int v=0; v<4; v++) 
			{
				verts[v*2] = chunk.verts[ face.cornerNums[v] ] + addVector;
				verts[v*2+1] = chunk.verts[ face.sideNums[v] ] + addVector;
			}
			verts[8] = chunk.verts[ face.centerNum ] + addVector;
		...
		{ 
			...
			for (int i=0; i<cubeVerts.Length; i++) verts[i] = Vector3.Scale(cubeVerts[i],size);
		...
		{ 
			...
			for (int i=0; i<sphereVerts.Length; i++) verts[i] = sphereVerts[i]*radius;
		...
		{ 
			...
			if (verts.Length != polyUvs.Length)
			{
				verts = new Vector3[6];
				uvs = polyUvs;
				tris = polyTris;
		...
		{ 
			...
			if (verts.Length != planeUvs.Length)
			{
				verts = new Vector3[4];
				uvs = planeUvs;
				tris = planeTris;
			...
			switch (dir)
			{
				case 0: verts = new Vector3[] {new Vector3(x,y+1,z), new Vector3(x+1,y+1,z), new Vector3(x+1,y+1,z+1), new Vector3(x,y+1,z+1)}; break;
				case 1: verts = new Vector3[] {new Vector3(x,y,z), new Vector3(x+1,y,z), new Vector3(x+1,y,z+1), new Vector3(x,y,z+1)}; break;
				case 2: verts = new Vector3[] {new Vector3(x+1,y,z), new Vector3(x+1,y+1,z), new Vector3(x+1,y+1,z+1), new Vector3(x+1,y,z+1)}; break;
				case 3: verts = new Vector3[] {new Vector3(x,y,z), new Vector3(x,y+1,z), new Vector3(x,y+1,z+1), new Vector3(x,y,z+1)}; break;
				case 4: verts = new Vector3[] {new Vector3(x,y,z+1), new Vector3(x,y+1,z+1), new Vector3(x+1,y+1,z+1), new Vector3(x+1,y,z+1)}; break;
				case 5: verts = new Vector3[] {new Vector3(x,y,z), new Vector3(x,y+1,z), new Vector3(x+1,y+1,z), new Vector3(x+1,y,z)}; break;
			}
		...
		static public Highlight Create (VoxelandTerrain land)
		{
			...
			highlight.GetComponent<Renderer>().receiveShadows = false;
			...
			highlight.GetComponent<Renderer>().sharedMaterial = land.highlightMaterial; //setting material
		...
		static private int[] planeTris = {0,1,2,2,3,0};
