...
namespace UnityEditor
{
	internal class UBER_StandardShaderGUI : ShaderGUI
	{
		...
		private static class Styles
		{
			...
			public static GUIContent _TranslucencyColorText = new GUIContent("Color", "(RGB) - forward\n\n(A) - deferred realtime lights (x occlusion mask x Strength below = translucency output to G-buffer diffuse A channel)");
			public static GUIContent _TranslucencyStrengthText = new GUIContent("Strength", "In deferred this is occlusion mask multiplier (clamped 0..1) - written into G-buffer diffuse A channel, so keep it in reasonable range there.\n\nStrength can be also additionaly controlled per light's alpha color - configurable in UBER_StandardConfig.cginc");
			...
			public static GUIContent _TranslucencyDeferredLightIndexText = new GUIContent ("Deferred light index", "Select the translucency color index for deferred (refer to UBER_DeferredTranslucencyParams.cs attached to the camera)");
		...
		public void ShaderPropertiesGUI (Material material, MaterialProperty[] props)
		{
			...
			bool _AlbedoOcclusion_Used = occlusionMap.textureValue == null || _UVSecOcclusion.floatValue==1;
...
//				}
				...
				if (_MainShown.floatValue == 1 || _MainShown.hasMixedValue) {
					...
					if (_TwoLayers_Used) {
						m_MaterialEditor.TexturePropertySingleLine (_UVSecOcclusion.floatValue == 0 ? Styles.occlusionText2 : Styles.occlusionText, occlusionMap, occlusionMap.textureValue != null && _UVSecOcclusion.floatValue == 0 ? occlusionStrength : null, null);
						if (occlusionMap.textureValue != null && _UVSecOcclusion.floatValue == 0) {
							ShaderPropertyTooltip (occlusionStrength2, Styles._SecOcclusionStrengthText, 2);
						}
					} else {
						m_MaterialEditor.TexturePropertySingleLine (Styles.occlusionText, occlusionMap, occlusionMap.textureValue != null && _UVSecOcclusion.floatValue == 0 ? occlusionStrength : null);
					}
		...
		static void SetMaterialKeywords(Material material, WorkflowMode workflowMode)
		{
			...
			if (!_DistanceMap_Used && !_ExtrusionMap_Used) {
				...
				SetKeyword (material, "_PARALLAX_MAP", flag_Parallax);
				SetKeyword (material, "_PARALLAX_MAP_2MAPS", flag_Parallax_2Maps);
			} else {
			...
			bool _AlbedoOcclusion_Used = (material.GetTexture("_OcclusionMap") == null || material.GetFloat("_UVSecOcclusion") == 1);
