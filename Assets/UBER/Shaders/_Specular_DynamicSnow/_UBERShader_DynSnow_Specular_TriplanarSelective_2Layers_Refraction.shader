Shader "UBER - DynSnow - Specular Setup/Refraction/Triplanar Selective - Two Layers"
{
	Properties
	{
        ...
        _TranslucencySuppressRealtimeShadows ("Translucency shadows suppression", Range(0,20)) = 0.5
		  [Enum(TranslucencyColor1,0,TranslucencyColor2,1,TranslucencyColor3,2,TranslucencyColor4,3)] _TranslucencyDeferredLightIndex ("Translucency deferred light index", Float) = 0
        ...
        [NoKeywordToggle] _WetnessLevelFromGlobal  ("Water level controlled globally", Float) = 1
		[NoKeywordToggle] _WetnessConstFromGlobal  ("Water const controlled globally", Float) = 1
        ...
        [NoKeywordToggle] _SnowLevelFromGlobal ("Snow level controlled globally", Float) = 1
		[NoKeywordToggle] _FrostFromGlobal ("Frost controlled globally", Float) = 1
        [NoKeywordToggle] _SnowBumpMicroFromGlobal ("Snow bumps micro controlled globally", Float) = 1
        [NoKeywordToggle] _SnowDissolveFromGlobal ("Snow dissolve controlled globally", Float) = 1
        [NoKeywordToggle] _SnowSpecGlossFromGlobal ("Snow spec/gloss controlled globally", Float) = 1
	...
	CGINCLUDE
		...
		// (remove shader features keywords on detail, we have no POM on 2 layers, but only shader_feature _PARALLAX_MAP _PARALLAX_MAP_2MAPS)
		#define _TWO_LAYERS
	...
	SubShader
	{
		...
		Pass
		{
			...
			//#pragma shader_feature ___ _PARALLAX_MAP
			// keywords available in 2 layers mode
			#pragma shader_feature ___ _PARALLAX_MAP _PARALLAX_MAP_2MAPS
		...
		*/			
		...
		Pass
		{
			...
			// keywords available in regular mode
			//#pragma shader_feature ___ _PARALLAX_MAP _PARALLAX_POM _PARALLAX_POM_ZWRITE _PARALLAX_POM_SHADOWS
			// keywords available in 2 layers mode
			#pragma shader_feature ___ _PARALLAX_MAP _PARALLAX_MAP_2MAPS
			// 
			#pragma shader_feature _TRIPLANAR_WORLD_MAPPING
		...
		Pass
		{
			...
			// keywords available (_PARALLAX_MAP_2MAPS in 2 layers mode only)
			#pragma shader_feature ___ _PARALLAX_MAP _PARALLAX_MAP_2MAPS
			// we don't have it in 2-layers mode
			//#pragma shader_feature ___ _DETAIL_SIMPLE _DETAIL_TEXTURED _DETAIL_TEXTURED_WITH_SPEC_GLOSS
			#pragma shader_feature _TRIPLANAR_WORLD_MAPPING
