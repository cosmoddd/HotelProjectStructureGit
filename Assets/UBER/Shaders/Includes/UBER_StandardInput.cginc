...
#if (_NORMALMAP || !DIRLIGHTMAP_OFF || _PARALLAX_MAP || _PARALLAX_MAP_2MAPS || _PARALLAX_POM || _PARALLAX_POM_ZWRITE || _PARALLAX_POM_SHADOWS || defined(DISTANCE_MAP) || defined(EXTRUSION_MAP) || !defined(_WETNESS_NONE) || defined(_SNOW))
...
half4 tex2Db(sampler2D _Tex, float2 _uv, float2 _ddx, float2 _ddy, half bias) {
	//#if defined(POM) || defined(_PARALLAX_MAP) || defined(_PARALLAX_MAP_2MAPS) || defined(DISTANCE_MAP) || defined(EXTRUSION_MAP)
...
half3 Albedo(float4 texcoords, fixed4 vertex_color, half Wetness, float2 _ddx, float2 _ddy, float2 _ddxDet, float2 _ddyDet, inout half3 _emission, half blendFade) // UBER - additional params
{
	#if defined(TRIPLANAR_SELECTIVE)
		half3 albedo = _Color.rgb * lerp( _MainTexAverageColor.rgb, tex2Dp(_MainTex, texcoords.xy, _ddx, _ddy).rgb, saturate(blendFade+_MainTexAverageColor.a) );
	#else
		half3 albedo = _Color.rgb * tex2Dp(_MainTex, texcoords.xy, _ddx, _ddy).rgb;
	#endif
#if defined(_TWO_LAYERS)
	#if defined(TRIPLANAR_SELECTIVE)
		albedo = lerp(_Color2.rgb * lerp( _MainTex2AverageColor.rgb, tex2Dp(_MainTex2, texcoords.zw, _ddxDet, _ddyDet).rgb, saturate(blendFade+_MainTex2AverageColor.a)), albedo, __VERTEX_COLOR_CHANNEL_LAYER);
	#else
		albedo = lerp(_Color2.rgb * tex2Dp(_MainTex2, texcoords.zw, _ddxDet, _ddyDet).rgb, albedo, __VERTEX_COLOR_CHANNEL_LAYER);
	#endif
#else
	...
	#else
		half mask = DetailMask(texcoords.xy, vertex_color, _ddx, _ddy); // UBER - vertex color added
	#endif
	#if _DETAIL
		// textured
...
// UBER - detail spec/gloss
	...
	#if _DETAIL || defined(_DETAIL_SIMPLE)
		half mask = DetailMask(uv.xy, vertex_color, _ddx, _ddy); // UBER - detail mask per vertex
	#endif
	#if defined(_DETAIL_SIMPLE)
...
half3 NormalInTangentSpace(half3 _normFromParallaxFunction, float4 texcoords, float2 uvDet_no_refr, half3 i_viewDirForParallax, half _snow_val_nobump, half3x3 tanToWorld, fixed4 vertex_color, float2 _ddx, float2 _ddy, float2 _ddxDet, float2 _ddyDet, float2 _ddxSnow, float2 _ddySnow, half3 wetNorm, half deepWetFct, inout half _snow_val, inout half dissolveMaskValue, half blendFade) // UBER - vertex_color, derivatives, wetNorm, deepWetFct
#else
...
#endif
{
	...
	half mask = DetailMask(texcoords.xy, vertex_color, _ddx, _ddy); // UBER - detail mask per vertex
...
//	#endif
	...
	snowNormal.xy *= lerp(sv*6*dissolveMaskValue, 1, _snow_val) * (_SnowBumpMicroFromGlobal ? _UBER_GlobalSnowBumpMicro : _SnowBumpMicro);
