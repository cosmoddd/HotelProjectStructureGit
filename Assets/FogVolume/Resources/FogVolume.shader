    ...
    float4 frag (v2f i) : COLOR
      {
        ...
        CloudFade = min(1,  exp( -CloudFade ));
					...
					float3 Layer1_Coords = SampleCoordinates * (_3DNoiseScale * Stretch.rgb) + Speed.rgb;
								...
								Noise = Threshold(lerp(tex3D(_NoiseVolume,  Layer1_Coords).r ,	tex3D(_NoiseVolume,  Layer2_Coords ).r, .5), NoiseAtten, threshold) * float4(Gradient.xyz, 1.0);
						#else
								Noise = Threshold(tex3D(_NoiseVolume,  Layer1_Coords).r, NoiseAtten, threshold) * float4(Gradient.xyz, 1.0);
